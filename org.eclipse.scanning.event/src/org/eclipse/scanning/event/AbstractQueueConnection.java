...
public abstract class AbstractQueueConnection<U extends StatusBean> extends AbstractConnection implements IQueueConnection<U>{
	...
	/**
	 * Roll over the queue removing messages and re-adding them until we get to the first one we started with.
	 * This is probably not algorithmically correct because new messages could come in at a different order.
	 * 
	 * @param bean
	 * @throws Exception 
	 */
	protected void updateQueue(String queueName, U bean) throws EventException {
		
		if (bean.getUniqueId()==null) throw new EventException("The bean must have a unique id!");
		...
		try {
			...
			while(e.hasMoreElements()) {
				...
				TextMessage t = (TextMessage)rem;
				final U b = service.unmarshal(t.getText(), getBeanClass());
				MessageProducer producer = qSes.createProducer(queue);
				if (b.getUniqueId().equals(bean.getUniqueId())) {
					
					b.setStatus(bean.getStatus());
					t = qSes.createTextMessage(service.marshal(b));
					
					t.setJMSMessageID(bean.getUniqueId());
					t.setJMSExpiration(t.getJMSExpiration());
					t.setJMSTimestamp(t.getJMSTimestamp());
					t.setJMSPriority(t.getJMSPriority());
				}
				producer.send(t);
				producer.close();
			...
			throw new EventException("Cannot update bean in list! "+queueName, ne);
			...
			if (qCon!=null) {
				try {
					...
					throw new EventException("Cannot close connection! "+queueName, e);
				}
			}
		}
