...
public class MandelbrotExamplePluginTest {
	...
	private void checkNexusFile(IRunnableDevice<ScanModel> scanner,
		...
		final ScanModel scanModel = ((AbstractRunnableDevice<ScanModel>) scanner)
				.getModel();
		assertEquals(DeviceState.READY, scanner.getDeviceState());

		String filePath = ((AbstractRunnableDevice<ScanModel>) scanner)
				.getModel().getFilePath();
		...
		assertEquals(nxDataGroups.size(), 2);
		for (String nxDataGroupName : nxDataGroups.keySet()) {
			...
			// Check axes
			final IPosition pos = scanModel.getPositionIterable().iterator()
					.next();
			...
			if (sourceFieldName.equals(NXdetector.NX_DATA)) {
				// add placeholder value "." for each dimension of image
				expectedAxesNames.addAll(Collections.nCopies(2, "."));
			}
			...
			// validate the additional NXdata for the 'total' dataset
			nxData = entry.getData(detectorName + "_total");
			assertNotNull(nxData);
			assertSignal(nxData, "total");
			assertSame(nxData.getDataNode("total"),
					detector.getDataNode("total"));
		}
	}
